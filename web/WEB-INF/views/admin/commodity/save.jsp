  ...
  <div class="content">
    ...
    <form class="am-form view-save" id="f1" action="" method="POST"  enctype="multipart/form-data" request-type="ajax-url">
      ...
      <div class="am-form-group">
        <label>分类</label>
        <select name="cid" class="am-radius c-p" data-validation-message="请选择所在地区" required>
          <option value="">可选择...</option>
          <c:forEach items="${classifyList}" var="cs">
            <c:choose>
              <c:when test="${cs.id == shop.cid}">
      ...
      <div class="am-form-group">
        <label>商品名称 </label>
        <input type="text" value="${shop.name}" name="name" placeholder="商品名称" minlength="2" maxlength="16" data-validation-message="姓名格式 2~16 个字符" class="am-radius"  required />

      </div>
      <div class="am-form-group">
        <label>商品简介</label>
        <textarea name="description" required>${shop.description}</textarea>
      </div>
    ...
    $.ajax({
      ...
      success:function(data){
        ...
        if(data.code==1){
          ...
          layer.msg(data.msg);
          setTimeout(function(){
            window.location.href='/commodity/save';
          },2000);
        }
      }
    })
